...
class Activate extends Generic {
	...
	function __construct() {
		...
		if(isset($_SESSION['loggedUser']['username']))
			$this->user = $_SESSION['loggedUser']['username'];
	...
	private function getKey() {
		...
		if(!empty($_SESSION['loggedUser']['activate'])) unset($_SESSION['loggedUser']['activate']);
	...
	private function resendKey() {
		...
		$shortcodes = array(
			...
			'full_name'		=>	$row['name'],
			'username'		=>	$this->user,
			'activate'		=>	SITE_PATH . "?login=activate.php&key=$key"
		);

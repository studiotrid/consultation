<?php
	...
	if(empty($_SESSION['loggedUser']['username'])) { header('Location: login.php'); exit(); }
	...
	if(empty($error)) { if(get_magic_quotes_gpc()) { $comments = stripslashes($comments); }
	 ...
	 if(!isset($address)) $address = "studio.trid@gmail.com";
...
?>
	...
	<p><?php  _e('Fill out this form if you feel this is in error'); ?></p>
		...
		<form method="post" action="?login=disabled.php">

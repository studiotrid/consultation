 ...
 *
 * @author       loggedUser <info@loggedUser.co.uk>
 * @author       Matt Gates <matt.gates@jigoshop.com>
 * @copyright    Copyright © 2009-2012 loggedUser Ltd.
...
class loggedUser_integration extends Generic {
	...
	function __construct() {
		...
		if (in_array('twitter', $this->enabledMethods)) {
			require ROOT.'login/classes/integration/twitter/EpiCurl.php';
			require ROOT.'login/classes/integration/twitter/EpiOAuth.php';
			require ROOT.'login/classes/integration/twitter/EpiTwitter.php';
		}
		...
		if (in_array('facebook', $this->enabledMethods)) {
			require ROOT.'login/classes/integration/facebook/facebook.php';
		}
		...
		if (in_array('google', $this->enabledMethods) || in_array('yahoo', $this->enabledMethods)) {
			include_once( ROOT.'login/classes/integration/openid/openid.php' );
		}
	...
	private function retrieveInfo() {
		...
		$params = array( ':user_id' => $_SESSION['loggedUser']['user_id'] );
	...
	public function link_account($link, $login = false) {
		...
		if ( !empty($_SESSION['loggedUser'][$link]) ) {
	...
	private function connect($link) {
		...
		$params = array(
			...
			':session_link' => $_SESSION['loggedUser'][$link]
		);
	...
	private function unlink($provider) {
		...
		unset($_SESSION['loggedUser'][$provider]);
			...
			if(!$openid->mode) {
				...
				if ($openid->validate()) :
					...
					if( !empty($providers[$link]) ) :
						...
						$_SESSION['loggedUser'][$link] = $openid->identity;
						$this->connect($link);
						$_SESSION['loggedUser']['openIDMisc'] = $openid->getAttributes();
						$_SESSION['loggedUser']['openIDMisc']['type'] = $link;
						$_SESSION['loggedUser']['openIDMisc'][$link] = $openid->identity;
	...
	public function facebook_url() {
		...
		if( !empty($_SESSION['loggedUser']['facebook']) )
			return false;
		...
		if ($this->fbUser) :
		  ...
		  try {
			$_SESSION['loggedUser']['facebookMisc'] = $user_profile;
			$_SESSION['loggedUser']['facebook'] = $this->fbUser;
	...
	public function twitter_url() {
		...
		if ( !empty($_SESSION['loggedUser']['twitter']) )
			return false;
		...
		if( !empty($_GET['oauth_token']) ) {
			...
			$_SESSION['loggedUser']['twitter'] = $twitterInfo['id'];
			$_SESSION['loggedUser']['twitterMisc']['username'] = $twitterInfo['screen_name'];
			$_SESSION['loggedUser']['twitterMisc']['id'] = $twitterInfo['id'];
			$_SESSION['loggedUser']['twitterMisc']['name'] = $twitterInfo['name'];

		}

		return !empty($_SESSION['loggedUser']['twitter']) ? '#' : $this->twitterObj->getAuthenticateUrl( null,array('oauth_callback' => str_replace("www.","", SITE_PATH) . basename($_SERVER['PHP_SELF']) . '?link=twitter' ));
...
$loggedUser_integration = new loggedUser_integration();
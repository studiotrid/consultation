...
include_once( ROOT.'login/classes/translate.class.php' );
include_once( ROOT.'login/classes/connect.class.php' );
...
class Generic extends Connect {
	...
	function __construct() {
		...
		include( ROOT.'login/classes/prereqs.php' );
		...
		if(empty($this->error)) include_once( ROOT.'login/classes/upgrade.class.php' );
	...
	public function denyAccessLogs() {
		...
		return ( ($this->getOption('profile-timestamps-admin-enable') && !in_array(1, $_SESSION['loggedUser']['user_level'])) || !$this->getOption('profile-timestamps-enable') );
	...
	public function generateToken() {
		...
		if(empty($_SESSION['loggedUser']['token']))
			$_SESSION['loggedUser']['token'] = md5(uniqid(mt_rand(),true));
	...
	public function valid_token($token) {
		...
		if ($_SESSION['loggedUser']['token'] != $token)
			return false;
	...
	public function definePaths() {
		...
		if (!defined('address'))                define( 'address',             ADMIN_EMAIL );
		if (!defined('SITE_PATH'))              define( 'SITE_PATH',           ADMINPATH );

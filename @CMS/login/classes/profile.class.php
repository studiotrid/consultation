...
class Profile extends Generic {
	...
	function __construct() {
		...
		if(!empty($_SESSION['loggedUser']['forcePwUpdate']))
		...
		// Save the username
		$this->username = !empty($_SESSION['loggedUser']['username']) ? $_SESSION['loggedUser']['username'] : _('Guest');
	...
	private function determineGuest() {
		...
		if ( !empty($_SESSION['loggedUser']['user_id']) && empty($_GET['uid']) )
			$this->user_id = $_SESSION['loggedUser']['user_id'];
		...
		$this->guest = !( !empty($_SESSION['loggedUser']['user_id']) && $_SESSION['loggedUser']['user_id'] == $this->user_id );
	...
	private function updateEmailorPw() {
		...
		if(!empty($_SESSION['loggedUser']['forcePwUpdate'])) unset($_SESSION['loggedUser']['forcePwUpdate']);
	...
	private function process() {
		...
		if ( !empty($this->settings['password']) || $this->settings['email'] != $email ) :
			...
			$shortcodes = array(
				...
				'full_name'     =>  $this->settings['name'],
				'username'      =>  $this->username,
				'confirm'       =>  SITE_PATH . "login=profile.php&key=$key"
			);

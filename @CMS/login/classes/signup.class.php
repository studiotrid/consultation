...
class SignUp extends Generic {
	...
	public function doCaptcha($display = true) {
		...
		switch ( $captcha ) :
			...
			case 'reCAPTCHA' :
				...
				require_once(ROOT.'login/classes/captcha/recaptcha-1.11/recaptchalib.php');
			...
			case 'playThru' :
				...
				require_once(ROOT.'login/classes/captcha/ayah-1.0.2/ayah.php');
	...
	public function getPost($var) {
		...
		if ( !empty($_SESSION['loggedUser']['openIDMisc']) ) :

			$misc = $_SESSION['loggedUser']['openIDMisc'];
		...
		if ( !empty($_SESSION['loggedUser']['facebookMisc']) ) :

			$misc = $_SESSION['loggedUser']['facebookMisc'];
		...
		if ( !empty($_SESSION['loggedUser']['twitterMisc']) ) :

			$misc = $_SESSION['loggedUser']['twitterMisc'];
	...
	private function register() {
		...
		if(empty($this->error)) {
			...
			$_SESSION['loggedUser']['username'] = $this->settings['username'];

			/* Apply default user_level */
			$_SESSION['loggedUser']['user_level'] = unserialize(parent::getOption('default-level'));

			if ( $requireActivate )
				$_SESSION['loggedUser']['activate'] = 1;

			$_SESSION['loggedUser']['gravatar'] = parent::get_gravatar($this->settings['email'], true, 26);
			...
			if ( !empty($_SESSION['loggedUser']['facebookMisc']) ) {
				$link = 'facebook';
				$id = $_SESSION['loggedUser']['facebookMisc']['id'];
			}

			if ( !empty($_SESSION['loggedUser']['openIDMisc']) ) {
				$link = $_SESSION['loggedUser']['openIDMisc']['type'];
				$id = $_SESSION['loggedUser']['openIDMisc'][$link];
			}

			if ( !empty($_SESSION['loggedUser']['twitterMisc']) ) {
				$link = 'twitter';
				$id = $_SESSION['loggedUser']['twitterMisc']['id'];
			}

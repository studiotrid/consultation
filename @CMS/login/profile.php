<?php include_once(ROOT.'login/classes/profile.class.php');?>
...
<div class="tabs-left">
	...
	<ul class="nav nav-tabs">
		...
		<?php if ( !$profile->guest && !empty( $loggedUser_integration->enabledMethods ) ) : ?>
		<li><a href="#usr-integration" data-toggle="tab"><i class="icon-random"></i> <?php _e('Integration'); ?></a></li>
		<?php endif; ?>

	</ul>

	<form class="form-horizontal" method="post" action="?login=profile.php">
		...
		<div class="tab-pane fade in active" id="usr-control">
			<fieldset>
				...
				<div class="control-group">
					...
					<div class="controls">
						<span class="uneditable-input"><?php echo SITE_PATH . '?login=profile.php&uid=' . $profile->getField('user_id'); ?></span>
					</div>
				</div>
				<?php endif; ?>
		...
		<div class="tab-pane fade" id="usr-integration">
			<fieldset>
				...
				<?php
					...
					foreach ($loggedUser_integration->enabledMethods as $key ) :
						$inUse = $loggedUser_integration->isUsed($key);
						?><div class="span3">
							<a class="a-tooltip" href="#" data-rel="tooltip" tabindex="99" title="<?php echo ucwords($key); ?>">
								<img src="<?=ROOT;?>login/assets/img/<?php echo $key; ?>.png" alt="<?php echo $key; ?>">
							</a>

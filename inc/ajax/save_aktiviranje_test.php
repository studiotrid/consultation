<?php
/**
 * AJAX handler for saving activation test results
 */
...
try {
    $result = $db->update('karmicka_ispit_termini', $dataj, 'ID="' . $ispitId . '"');
    if ($result !== false) {
        // Get the test record to format the success text
        $testRecord = $db->query_first("SELECT datum FROM karmicka_ispit_termini WHERE ID='" . $ispitId . "'");
        $uspehText = '';

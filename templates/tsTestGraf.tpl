    ...
    <div class="testTS">{$tsTest.vreme|date_format:"%d.%m.%Y. "} <font style="color:">{$tsTest.nazivPlanete}</font><br /></b></div>
    ...
    <div class="grafikon" style="">
        ...
        <table class="table main" style="margin-left:30px;" id="tabelas{$smarty.foreach.dddd.iteration}">
        <thead>
        <tr class="headerRow" style="display:none"><th>Pitanje</th><th>Odgovor</th><th>Planeta</th><th>Esencija</th><th>Nivo</th></tr>
        </thead>
        <tbody>
        {foreach from=$tsTest.stubovi item=stubsa}
